...
import { BUSINESS, REELS, SERVICES, HERO_IMAGE, CTA_IMAGE } from "@/lib/config";
...
/* `refCode` is set when this is rendered under /r/<code>. It bakes the
   detailer's code straight into every SMS link server-side, so a customer
   who taps before the page finishes loading still gets tagged. */
export default function HomeContent({ refCode }: { refCode?: string }) {
  const smsHref = refSmsHref(refCode);
  ...
  return (
    <main className="min-h-screen bg-[#050506]">
      ...
      <section className="py-24 sm:py-32">
        ...
        <ServiceShowcase smsHref={smsHref} />
      </section>

      {/* ═══════════════════ BRING BLACK BACK ═══════════════════ */}
      <BringBlackBack smsHref={smsHref} />
