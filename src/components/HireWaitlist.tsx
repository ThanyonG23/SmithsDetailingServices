...
/* Vehicle-hire waitlist. Posts to /api/waitlist tagged source="vehicle-hire"
   so hire leads land in the ops dashboard, separate from detailing/membership. */
export default function HireWaitlist() {
    ...
    try {
      const res = await fetch("/api/waitlist", {
        ...
        body: JSON.stringify({
          ...
          membership: false,
          source: "vehicle-hire",
          interests: ["vehicle-hire"],
          message: need ? `Hire need: ${need}` : "",
        }),
      });
  ...
  return (
    <div className="rounded-3xl border border-white/12 bg-white/[0.03] p-6 sm:p-8">
      <h3 className="font-display text-2xl font-extrabold tracking-tight text-white">Join the hire waitlist</h3>
      <p className="mt-2 text-sm leading-relaxed text-white/60">
        Leave your details and we&apos;ll message you the moment our vehicles are available. No spam, no commitment.
      </p>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className={labelCls}>What would you need it for? (optional)</label>
          <input className={field} value={need} onChange={(e) => setNeed(e.target.value)} placeholder="e.g. weekend away, work ute" />
        </div>
      </div>
      ...
      <p className="mt-3 text-center text-xs text-white/35">We&apos;ll only message you about vehicle hire.</p>
    </div>
  );
}
